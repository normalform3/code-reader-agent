...
- 代码库总览卡片：项目概述、项目是什么、主要做什么和置信度。
- 项目说明书卡片：总览、技术栈、模块作用、项目入口、真实目录树和关键目录解释。
- 目录理解：用户点击目录树中的模块级目录后，右侧显示该目录作用；目录解释只到模块级，不展开到具体文件。
- 技术栈作用说明。
- Planner 计划。
- Skill Registry 选择结果。
- Context Snapshot。
...
8. 优先使用 `/api/agent/ask/stream` 接收 SSE 进度；等待期间直接在聊天气泡中显示 Agent trace，完成后展示最终回答、相关文件和上下文信息。
