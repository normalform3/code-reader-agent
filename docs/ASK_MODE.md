...
`AskModeRequest` 可携带 `conversation_id`。携带时，后端从该 Ask conversation 读取 `Session Memory`，本轮结束后只把新的消息和 `Session Memory` 写回该 conversation，不写入 project-level legacy session memory。
