...
- 使用 Project Memory 保存项目定位、技术栈、模块、文件、API 和流程候选。
- 使用 Session Memory 支持跨轮追问。
...
- 新增 `/api/agent/ask`。
- 支持项目总览、模块解释、文件解释、调用链、接口、配置和技术栈意图分类。
- 优先检索 Project Memory、Module Summary、File Summary、API Index、Flow Index 和 Session Memory。
- 使用 LangGraph `StateGraph` 表达 `IntentClassifier -> ContextRetriever -> ToolPlanner -> EvidenceCollector -> AnswerComposer -> MemoryUpdater`。
- 当记忆不足时调用只读工具补证据。
