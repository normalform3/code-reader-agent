...
`project_manual` 是首次自动生成的项目说明书，固定包含项目总览、技术栈说明、模块作用、项目入口、真实目录树、关键目录解释、evidence 和 uncertainties。项目总览优先由 LLM 基于只读工具上下文生成：先读取 README 判断信息是否足够；README 不足或缺失时，再在预算内调用 `build_repo_map` 和少量 `read_file` 补充上下文。报告生成后，系统会从 `project_manual` 和 Repo Map 派生 `project_memory`；后续追问通过 `/api/agent/ask` 检索 Project Memory 和 Session Memory，不再把每轮追问都当作完整报告重新生成。

边界：

- LLM 不可用时仍返回完整结构。
- LLM 总览生成触发上下文预算、工具次数或读取文件次数熔断时，返回 deterministic fallback 总览并记录 warnings/trace。
...
- 如果 LLM 不可用、超步数、输出不合法或触发上下文预算熔断，系统降级到 deterministic fallback，但仍返回 Phase 5.0 的完整分析任务结构。
- 代码库总览 final JSON 可返回 `project_summary`，后端校验通过后覆盖 `project_manual.overview`；校验失败时继续使用 Repo Map 的确定性 `project_summary`。
