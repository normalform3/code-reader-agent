...
DEFAULT_SKILL_DEFINITIONS = [
    ...
    },
    {
        "id": "VueSkill",
        "name": "VueSkill",
        "description": "分析 Vue/Vite 前端入口、路由、组件和状态管理线索。",
        "notes": "当 Repo Map 检测到 Vue、Vite、Vue Router 或 Pinia 时启用。",
        "details": [
            {"title": "适用场景", "items": ["Vue/Vite 前端项目结构、页面、组件、路由和状态管理分析。"]},
            {"title": "触发条件", "items": ["检测到 Vue、Vite、Vue Router 或 Pinia。"]},
            {"title": "优先读取文件", "items": ["vite.config.*", "src/main.*", "src/App.vue", "src/router/*", "src/views/*", "src/pages/*", "src/components/*", "src/stores/*"]},
            {"title": "可用 tools", "items": ["scan_project", "build_repo_map", "read_file", "search_code"]},
            {"title": "输出格式", "items": ["前端入口", "路由线索", "页面目录", "组件目录", "状态管理线索", "推荐阅读顺序", "依据文件"]},
    ...
    },
    {
        ...
        "description": "分析 Java/Spring Boot 入口、Controller、Service 和 Repository 线索。",
        "notes": "当 Repo Map 检测到 Java、Maven、Gradle 或 Spring 相关栈时启用。",
        "details": [
            {"title": "适用场景", "items": ["Java/Spring Boot 项目入口、后端分层、Controller/Service/Repository 阅读路径。"]},
            {"title": "触发条件", "items": ["检测到 Java、Maven、Gradle、Spring Boot、Spring Web 或 Spring Security。"]},
            {"title": "优先读取文件", "items": ["pom.xml", "build.gradle", "settings.gradle", "*Application.java", "*Controller.java", "*Service.java", "*Repository.java", "application.yml/properties"]},
            {"title": "可用 tools", "items": ["scan_project", "build_repo_map", "read_file", "search_code"]},
            {"title": "输出格式", "items": ["应用入口", "配置入口", "Controller 候选", "Service 候选", "Repository 候选", "阅读路线", "依据文件"]},
