...
用户在 Web UI 中输入公开 GitHub 仓库链接后，系统会把仓库导入到本地只读缓存，再触发扫描并查看 Repo Map 和 Agent 解释。已有本地项目路径仍作为内部扫描契约保留。
...
- Web UI 已支持输入公开 GitHub 仓库链接、触发导入和扫描、展示技术栈、入口文件、模块卡片、模块详情、工具调用、证据和提醒。
